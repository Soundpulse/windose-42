...
const METRO_LINES: MetroLine[] = [
  ...
  { id: 'asia_north_s', color: '#aaaaaa', stations: ['zhuhai', 'shenzhen', 'guangzhou', 'guilin', 'yunnan', 'chongqing', 'tibet'] },
  { id: 'se_asia', color: '#888888', stations: ['beijing', 'asia_elb_k', 'korea', 'japan', 'macau', 'hk', 'yunnan', 'thailand', 'philippines', 'sg', 'malay'] },
  { id: 'se_asia_tw', color: '#888888', stations: ['hk', 'taiwan'] },
  { id: 'euro', color: '#999999', stations: ['iceland', 'eur_elb0', 'edinburgh', 'manchester', 'uk', 'cambridge', 'harrogate', 'london', 'france', 'poland', 'eur_elb_p', 'portugal', 'spain'] },
  { id: 'euro_ext', color: '#999999', stations: ['portugal', 'italy', 'vatican'] },
  { id: 'usa', color: '#bbbbbb', stations: ['sf', 'usa_elb1', 'la', 'usa_elb2', 'sd', 'usa_elb3', 'vegas', 'usa_elb_v', 'slc', 'denver'] },
  { id: 'east_coast', color: '#cccccc', stations: ['buffalo', 'ec_elb_b', 'ithaca', 'ny', 'dc', 'philly', 'miami', 'ec_elb_h', 'hawaii'] },
];
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      METRO_LINES.forEach(line => {
        const points = line.stations.map(s => POSITIONS[s]).filter(Boolean).map(toCanvas);
        if (points.length < 2) return;
        ...
        if (!currentHover || isHoveredLine) {
          ...
          if (idx < points.length - 1) {
            ...
            const tx = p1.x + (p2.x - p1.x) * lt, ty = p1.y + (p2.y - p1.y) * lt;
            ctx.save();
            ctx.translate(tx, ty);
      ...
      LOCATIONS.forEach(loc => {
        const pos = POSITIONS[loc.id]; if (!pos) return;
    ...
    <div className="h-full w-full bg-black flex flex-col select-none overflow-hidden relative" ref={containerRef}>
      <header className="px-5 py-4 bg-black border-b border-gray-800 flex justify-between items-center z-50">
        ...
        <div className="text-[10px] font-mono text-gray-400 tracking-wider uppercase">
          {LOCATIONS.length} Active Nodes // V4.2
        </div>
      </header>
      ...
      <div className="flex-1 relative cursor-crosshair">
        <canvas 
          ...
          onMouseLeave={() => { setHoveredLoc(null); hoveredLocRef.current = null; }} 
