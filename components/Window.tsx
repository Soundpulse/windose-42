...
const Window: React.FC<WindowProps> = ({ 
  id, title, children, zIndex, isMinimized, isMobile,
  onClose, onFocus, onMinimize, icon 
}) => {
  const getInitialWidth = () => {
    switch(id) {
      case 'stats': return 380;
      case 'terminal': return 600;
      case 'chat': return 700;
      default: return 800;
    }
  };

  const [size, setSize] = useState({ width: getInitialWidth(), height: 500 });
  ...
  return (
    <div 
      ...
      style={getStyles()}
      onClick={() => { onFocus(); setIsFocused(true); }}
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          <span className="text-[10px] font-mono text-gray-400 truncate tracking-wider uppercase">
            {title}
          </span>
        </div>
      ...
      {/* Content */}
      <div className="flex-1 overflow-auto bg-[#0a0a0a] relative retro-scroll">
        {children}
      </div>
