...
import { User, Folder, Terminal, Activity, MessageCircle, Map, Github, Linkedin, Power, Trophy, Layers } from 'lucide-react';
    ...
    return (
      <div className="fixed inset-0 bg-black flex flex-col items-center justify-center z-[1000]">
        <div className="relative z-10 text-center">
          <div className={`mb-8 transition-all duration-500 ${bootPhase >= 1 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <div className="text-gray-600 font-mono text-[10px] tracking-[0.2em] mt-2">
              Toby's Personal Hub
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="pt-16 p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-flow-col md:grid-rows-6 gap-4 w-fit h-fit relative z-[5]">
        ...
        <DesktopIcon icon={<Activity size={20} />} label="System" onClick={() => openApp('stats')} />
        ...
        <DesktopIcon icon={<Github size={20} />} label="GitHub" onClick={() => handleOpenLink('https://github.com/soundpulse')} />
        <DesktopIcon icon={<Linkedin size={20} />} label="LinkedIn" onClick={() => handleOpenLink('https://www.linkedin.com/in/toby-io/')} />
      </div>
      ...
      {windows.map((win) => {
        ...
        switch(win.id) {
          ...
          case 'terminal': content = <TerminalApp onClose={() => closeApp('terminal')} onOpenLink={handleOpenLink} />; break;
          case 'stats': content = <StatsApp />; break;
          case 'chat': content = <ChatApp />; break;
          case 'achievements': content = <AchievementsApp />; break;
          case 'stack': content = <StackApp />; break;
          case 'go_out': content = <GoOutApp onHoverLocation={setHoveredLocationId} />; break;
          default: content = <div className="p-4">Empty</div>;
        }
      ...
      {/* Location Inspector */}
      <div className={`fixed right-6 bottom-20 w-80 transition-all duration-200 pointer-events-none z-[100] ${hoveredLocation ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
        <div className="bg-black border border-gray-800 p-6 text-right">
          ...
          <div className="flex justify-end gap-2 mb-3">
            <span className="text-[8px] font-mono tracking-[0.1em] text-black bg-white px-2 py-0.5 uppercase">{hoveredLocation?.label}</span>
          </div>
